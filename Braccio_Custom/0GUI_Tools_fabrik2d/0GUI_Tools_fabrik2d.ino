...
const int amountOfLocations = 8;
const int locations[amountOfLocations][3] = {{0,0,0}, {300, 0, 0}, {200, 0, 0}, {250, 0, 0}, {0, 300, 0}, {0, 200, 0}, {0, 250, 0}};
int i = 0;
...
void fabrik_loop() {
  ...
  if (fabrik2D.solve2(x, y, z, lengths)) {
    ...
    float base = fabrik2D.getBaseAngle();
    float b_base = convertToBraccioBase(base);
    float shoulder = fabrik2D.getAngle(0) * 57296 / 1000;
    float b_shoulder = convertToBraccioShoulder(shoulder);
    float elbow = fabrik2D.getAngle(1) * 57296 / 1000;
    float b_elbow = convertToBraccioElbow(elbow);
    float wristPitch = fabrik2D.getAngle(2) * 57296 / 1000;
    float b_wristPitch = convertToBraccioWristPitch(wristPitch);
